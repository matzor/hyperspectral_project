...
2 components, we get an average pixel error of 47.8\% for MNF and 47.4\% for PCA, whilst with only one component, 
the error becomes 37.8\% and 37.2\% for MNF and PCA respectively. This implies that parts of the variance of 
...
work better for MNF. Using 1 component we get 89.3\% for MNF and 40.0\% for PCA, but 
when using $P = 2$ we get an error of 50.7\% for MNF and 46.3\% for PCA. It is worth noting 
that although PCA gives lower mean pixel error that the MNF gives colors that are more true 
to the original, compared to the PCA where the red balloon in the image now has turned yellow. 
The original unfiltered noisy image has a mean error of 42.3\%, thus only PCA with 1 component 
is able to actually decrease the mean pixel error. \todo{Use SPy mnf instead?} 
%Gives MUCH better result for case2_2. Comment on this? Redo whole last paragraph if using SPy
...
PCA better when no noise(?) \todo{when is PCA better than MNF?}
